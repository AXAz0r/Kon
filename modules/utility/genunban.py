...
def generate_log_embed(message, target, args):
    ...
    log_embed.set_author(name='A Member Has Been Unquarantine', icon_url=avatar)
    log_embed.add_field(name='☣ Unquarantine User',
        ...
        if message.mentions:
            ...
            if hierarchy_me:
                if hierarchy_permit(author, target):
                    ...
                    response = discord.Embed(title=f'✅ {target.display_name} has been unbanned from {channel}.', color=0x77B255)
